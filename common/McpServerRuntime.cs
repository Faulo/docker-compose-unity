...
internal static class McpActivation
{
    internal static bool Parse()
    {
        var value = Environment.GetEnvironmentVariable("COMPOSE_UNITY_MCP");
        return value switch
        {
            null or "" or "0" => false,
            "1" => true,
...
internal sealed class McpServerRuntime : IAsyncDisposable
{
    private readonly WebApplication application;
    private readonly UnityMcpController controller;
    private readonly CancellationTokenSource stopping;
    private int stopped;

    private McpServerRuntime(
    ...
    internal static async Task<McpServerRuntime> StartAsync(CancellationToken sidecarStopping)
    {
        ...
        try
        {
            ...
            {
                Args = [],
                ApplicationName = typeof(McpServerRuntime).Assembly.FullName
            });
...
[McpServerToolType]
internal sealed class UnityMcpTools(UnityMcpController controller)
{
    [McpServerTool(
        Name = "project_info",
        ReadOnly = true,
    ...
    [Description("Read a Docker-host Unity project's YAML and JSON metadata and return its root, product metadata, editor and project versions, major code and rendering settings, input handling, and complete package manifest without installing or launching Unity.")]
    public async Task<object> ProjectInfoAsync(
        [Description("Docker-daemon host path to an existing Unity project.")] string projectRoot,
    ...
    [McpServerTool(
        ...
        OpenWorld = false,
        UseStructuredContent = true)]
    [Description("Run Unity Test Runner modes for a Docker-host Unity project. Valid JUnit reports return compact pass/fail counts and failure details; infrastructure errors return the complete ordered Unity log.")]
    public async Task<object> RunTestsAsync(
        [Description("Docker-daemon host path to an existing Unity project.")] string projectRoot,
        [Description("One or more Unity test modes, such as EditMode, PlayMode, or a platform.")] string[] modes,
        CancellationToken cancellationToken) =>
    ...
    public async Task<object> ExecuteMethodAsync(
        [Description("Docker-daemon host path to an existing Unity project.")] string projectRoot,
        [Description("Fully qualified static Unity editor method name.")] string method,
        CancellationToken cancellationToken,
        [Description("Optional arguments forwarded without shell reinterpretation.")] string[]? arguments = null) =>
