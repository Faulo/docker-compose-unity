...
internal static class ProjectProbe
{
    ...
    internal static int Run(string projectRoot)
    {
        try
        {
            ...
            var versionLines = File.ReadAllLines(versionPath);
            var editorVersion = ReadValue(versionLines, "m_EditorVersion:")
                ?? throw new InvalidOperationException("ProjectSettings/ProjectVersion.txt does not contain m_EditorVersion");
            var editorVersionWithRevision = ReadValue(versionLines, "m_EditorVersionWithRevision:");
    ...
    private static string ReadRequiredSetting(IReadOnlyList<string> lines, string name) =>
        ReadSetting(lines, name)
        ?? throw new InvalidOperationException($"ProjectSettings/ProjectSettings.asset does not contain {name}");

    private static int ReadRequiredIntegerSetting(IReadOnlyList<string> lines, string name)
    {
        var value = ReadRequiredSetting(lines, name);
    ...
    {
        var prefix = $"  {name}:";
        var line = lines.FirstOrDefault(line => line.StartsWith(prefix, StringComparison.Ordinal));
        return line is null ? null : DecodeYamlScalar(line[prefix.Length..]);
    }

    private static Dictionary<string, string> ReadEnumMap(
        IReadOnlyList<string> lines,
        string name,
        Func<int, string> format)
    {
        var result = new Dictionary<string, string>(StringComparer.Ordinal);
        var section = FindSetting(lines, name);
        if (section < 0 || !string.IsNullOrWhiteSpace(lines[section][($"  {name}:").Length..]))
        {
            return result;
        }

        for (var index = section + 1; index < lines.Count && Indentation(lines[index]) > 2; index++)
        {
            var line = lines[index].Trim();
            var separator = line.LastIndexOf(':');
            if (separator <= 0
                || !int.TryParse(line[(separator + 1)..].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
            {
                continue;
            }
    ...
    private static Dictionary<string, GraphicsApiSettings> ReadGraphicsApis(IReadOnlyList<string> lines)
    {
        var result = new Dictionary<string, GraphicsApiSettings>(StringComparer.Ordinal);
        var section = FindSetting(lines, "m_BuildTargetGraphicsAPIs");
        if (section < 0)
        {
            return result;
        }
        ...
        for (var index = section + 1; index < lines.Count; index++)
        {
            ...
            else if (line.StartsWith("m_APIs:", StringComparison.Ordinal))
            {
                ...
                && int.TryParse(line["m_Automatic:".Length..].Trim(), out var value))
            {
                automatic = value != 0;
            }
        }
    ...
    private static void AddGraphicsApis(
        ...
        bool? automatic)
    {
        ...
        result[buildTarget] = new GraphicsApiSettings
        {
            Automatic = automatic ?? true,
            Apis = DecodeGraphicsApis(serializedApis)
        };
    }

    private static string[] DecodeGraphicsApis(string? serialized)
    {
        ...
        try
        {
            var bytes = Convert.FromHexString(serialized);
            if (bytes.Length % sizeof(int) != 0)
            {
                throw new FormatException();
            }

            var result = new string[bytes.Length / sizeof(int)];
    ...
    private static string RenderPipeline(string projectRoot, JsonNode manifest)
    {
        var path = Path.Combine(projectRoot, "ProjectSettings", "GraphicsSettings.asset");
        if (!File.Exists(path))
        {
            return "Unknown";
        }
        ...
        var configured = ReadSetting(lines, "m_CustomRenderPipeline");
        ...
        var universal = dependencies?.ContainsKey("com.unity.render-pipelines.universal") == true;
        var highDefinition = dependencies?.ContainsKey("com.unity.render-pipelines.high-definition") == true;
    ...
    private static string ApiCompatibility(int value, string editorVersion)
    {
        var majorText = editorVersion.Split('.', 2)[0];
        _ = int.TryParse(majorText, NumberStyles.None, CultureInfo.InvariantCulture, out var major);
        return value switch
        {
            1 => ".NET 2.0",
            2 => ".NET 2.0 Subset",
    ...
    private static string ScriptingBackend(int value) => value switch
    {
        0 => "Mono",
        1 => "IL2CPP",
        2 => ".NET",
    ...
    private static string ColorSpace(int value) => value switch
    {
        0 => "Gamma",
        1 => "Linear",
        _ => $"Unknown ({value})"
    ...
    private static string InputHandling(int value) => value switch
    {
        0 => "Legacy",
        1 => "InputSystem",
        2 => "Both",
    ...
    private static string GraphicsApi(int value) => value switch
    {
        0 => "OpenGL2",
        1 => "Direct3D9",
        2 => "Direct3D11",
    ...
    private static int FindSetting(IReadOnlyList<string> lines, string name)
    {
        var prefix = $"  {name}:";
    ...
    private static string? ReadRevision(string? editorVersionWithRevision)
    {
        ...
        var opening = editorVersionWithRevision.LastIndexOf('(');
        var closing = editorVersionWithRevision.LastIndexOf(')');
...
internal sealed class ProjectProbeResult
{
...
internal sealed class GraphicsApiSettings
{
